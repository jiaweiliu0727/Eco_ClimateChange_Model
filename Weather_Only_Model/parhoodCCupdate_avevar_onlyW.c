#include "headCC_Multi2_avevar.h"
...
//JL: Best parameters of the weather-only model, from Kyle et al. 2020
double bestparams[30]={1, 1, 1, 0, 0, 0, 0, 10, 5, 50, 0, 0.54426069999955, 0, 1.2295759999203, 0.855325949969273, 0, 0, 0, 0.100036849998988, 462.890999999309, 9.94603449930594, 1.5857159999382, 0, 0.247058999999818, 0, 8.44968999939637, 2534.67499226646, 107.258599999953, 279.75285, 0.137767249999265};

//Initial host densities at each location
double initialS[9] = {25.0, 25, 25.0, 25.0, 25.0, 25.0, 25.0, 25.0, 25.0};

//Initial resting spore densities, taken from Kyle et al. 2020. The 1st to 3rd values represent low to high fungus densities.
double initialR[9] = {0.01087467,0.1951949,0.3994755,0.01087467,0.1951949,0.3994755,0.01087467,0.1951949,0.3994755}; //sites:6,2,1,6,2,1,6,2,1
...
char strFileName[99];					// from filenames.h; name for the output file of mean of fraction infected at each location
GetString(pro,0,strFileName,98);		fflush(stdout);		//getc(stdin);
FILE *fp_results;

char strFileNameV[99];  //Name for the output file of variance of fraction infected at each location
GetStringV(pro,0,strFileNameV,98);       fflush(stdout);		//getc(stdin);
FILE *fp_results_v;
// ---------------------------------------- Random Number Stuff ------------------------------------------------------ //
gsl_rng *r_seed;
r_seed=random_setup();
...
for(k=1; k<2; k++){  //which population to run; host and resting spore densities do not matter in the weather-only model

	pop=k;

	Params.PARS[30+pop]=initialS[pop];  //CK//  I think these should just be initialS[pop], not initialS[pop-1].  Keep everything where pop starts at 1
	Params.PARS[50+pop] = initialR[pop];


    FILE *fp;
    char name[100];
    sprintf(name,"max_lhood/wonly_future_reps.txt");  //File name for the output of all values of fraction infected across all realizations
    fp=fopen(name,"a+");
    ...
    for(i=0;i<19800;i++){       //JL: Whole gypsy moth range 132*150 grids
		...
		//printf("test 2 here\n");//getc(stdin);
			...
			//if(MAXT3 > 126){MAXT3=126;}
			//if(MAXT3 < 21){MAXT3=21;}
